...
### 2. 고급설정
[고급 설정] 버튼을 클릭하면 다양한 설정을 할 수 있습니다.
![](images/rtsop_advanced_settings.PNG){: style="width:200px; display:block; margin:0 auto"}

|설정|설명|
|---|---|
| 진입매수율| 종가 대비 얼마나 하락해야 1티어를 매수할 지 결정합니다. 0으로 입력하면 종가 상관없이 무조건 1티어 매수합니다.|
| N 매도 후 중지 | 매도가 N회 발생하면 감시를 중지합니다. (사용안할 때는 0 입력)|
| N 매수 후 중지 | 매수가 N회 발생하면 감시를 중지합니다. (사용안할 때는 0 입력)|
| 티어정보 참조 슬롯 | 다른 슬롯의 티어 정보를 참조합니다. (예) 슬롯1 에서 본장, 슬롯2에서 주간장을 실행할 때 슬롯2에 1을 입력합니다. (슬롯1에는 입력하지 않음) |
| 미리주문| 체크가 되어 있으면 목표 매도가/매수가에 도달하지 않더라도 미리 주문을 넣습니다. 감시간격 사이에 목표가에 도달한 경우 체결이 됩니다.|
| 로스컷 비율 | 해당 값 이상으로 손실이 발생한 티어는 손절 처리합니다. 음수로 입력합니다. (사용안할 때는 0 입력)|
| 로스컷 발생시 종료 | 로스컷이 발생하면 감시를 중지합니다.|
